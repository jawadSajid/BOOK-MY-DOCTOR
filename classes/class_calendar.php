...
public $cost_per_slot				= 300;			// The cost per slot
public $cost_currency_tag			= "Rs.";		// The currency tag in HTML such as &euro; &pound; &yen;
...
function make_booking_array($year, $month, $j = 0) { 
	...
	if(isset($_SESSION['doc-id']))
	{
		$did=$_SESSION['doc-id'];
	}


	$stmt = $this->link->prepare("SELECT name, date, start FROM appointments WHERE date LIKE  CONCAT(?, '-', ?, '%') and did='$did'"); 
		...
		<div id='selected_slots'></div>		
			...
			<div id='basket_details'>
				...
				<form method='post' action='book_slots.php'>
					...
					<input name='doc-id' id='doc-id' type='text' value='$did' class='text_box'>

					<label>Patient ID</label>
					<input name='patient-id' id='patient-id' type='text' value='$pid' class='text_box'>
